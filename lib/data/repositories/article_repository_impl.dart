...
class ArticleRepositoryImpl extends DatabaseAccessor<AppDatabase> with _$ArticleRepositoryImplMixin, ArticleRepository {
  ...
  Stream<ArticleEntity> streamArticleById(int id) {
    return (select(articles)..where((t) => t.number.equals(id)))
        .watchSingle()
        .map((value) => ArticleEntity(number: value.number, text: value.value));
  }

  @override
  Stream<List<ArticleEntity>> streamArticles(List<int> ids) {
    return (select(articles)..where((t) => t.number.isIn(ids)))
        .watch()
        .map((values) => values
            .map((value) => ArticleEntity(number: value.number, text: value.value))
            .toList());
  ...
  Stream<List<ArticleEntity>> search(String query) {
    ...
    return (select(articles)
      ..where((t) => t.value.lower().contains(lowerQuery)))
        .watch()
        .map((values) => values
            .map((value) => ArticleEntity(number: value.number, text: value.value))
            .toList());
